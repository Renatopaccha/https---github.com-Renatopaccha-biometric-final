...
interface CalcSampleSizePropParams {
  N: number | null;
  proportion: number;
  confidence: number;
  precision: number;
...
interface CalcPrecisionPropParams {
  N: number | null;
  proportion: number;
  confidence: number;
  sampleSize: number;
...
interface CalcPrecisionRangePropParams {
  N: number | null;
  proportion: number;
  confidence: number;
  designEffect: number;
...
interface SampleSizeResult {
  type: 'sampleSize';
  value: number;
  params: { N: number | null; proportion: number; confidence: number; precision: number; designEffect: number };
}

interface PrecisionResult {
  type: 'precision';
  value: string;
  params: { N: number | null; proportion: number; confidence: number; sampleSize: number; designEffect: number };
}

interface RangeResult {
  type: 'range';
  values: RangeRow[];
  params: { N: number | null; proportion: number; confidence: number; designEffect: number; min: number; max: number; increment: number };
}
...
function calcSampleSizeProp({ N, proportion, confidence, precision, designEffect }: CalcSampleSizePropParams): number {
...
function calcPrecisionProp({ N, proportion, confidence, sampleSize, designEffect }: CalcPrecisionPropParams): number {
  const alpha = 1 - confidence / 100;
  const z = Math.abs(normalInv(alpha / 2));
  const p = proportion / 100;
  const q = 1 - p;
  ...
  if (N && N > 0) {
    ...
    return z * Math.sqrt((p * q * fpc) / nAdj) * 100;
  }
  return z * Math.sqrt((p * q) / nAdj) * 100;
}

function calcPrecisionRange({ N, proportion, confidence, designEffect, min, max, increment }: CalcPrecisionRangePropParams): RangeRow[] {
  const results: RangeRow[] = [];
  for (let prec = min; prec <= max + 0.0001; prec += increment) {
    const n = calcSampleSizeProp({ N, proportion, confidence, precision: prec, designEffect });
    results.push({ precision: prec, n });
  }
  return results;
}
...
function Field({ label, tooltip, value, onChange, placeholder, suffix, optional, disabled, hint }: FieldProps) {
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      ...
      {hint && (
        <p style={{ fontSize: 12, color: '#9ca3af', margin: '6px 0 0', lineHeight: 1.4 }}>{hint}</p>
      )}
    </div>
  );
}
...
interface ProportionVisualProps {
  value: string;
}

function ProportionVisual({ value }: ProportionVisualProps) {
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      ...
      <div style={{
        ...
        fontSize: 11, color: '#9ca3af', marginTop: 6, fontWeight: 500,
      }}>
        <span>p = {capped.toFixed(1)}% con el evento</span>
        <span>q = {(100 - capped).toFixed(1)}% sin el evento</span>
      </div>
    </div>
  );
}
...
export function ProportionCalculator({ onBack }: ProportionCalculatorProps) {
  ...
  useEffect(() => {
    if (mode === 'sampleSize') {
      if ((precision || precMin || precMax) && proportion && confidence) setCurrentStep(3);
      else if (proportion || confidence !== '95')                        setCurrentStep(2);
      else                                                               setCurrentStep(1);
    } else {
      if (sampleSizeInput && proportion && confidence) setCurrentStep(3);
      else if (proportion || confidence !== '95')      setCurrentStep(2);
      else                                             setCurrentStep(1);
    }
  }, [N, proportion, confidence, precision, precMin, precMax, sampleSizeInput, mode]);
    ...
    if (mode === 'sampleSize') {
      if (useRange) {
        ...
        const rangeResults = calcPrecisionRange({
          N: nNum, proportion: propNum, confidence: confNum,
          designEffect: deNum, min: minVal, max: maxVal, increment: incVal,
        });
        setResult({
          type: 'range', values: rangeResults,
          params: { N: nNum, proportion: propNum, confidence: confNum, designEffect: deNum, min: minVal, max: maxVal, increment: incVal },
        });
      } else {
        const precNum = parseFloat(precision);
        if (!precNum) return;
        const n = calcSampleSizeProp({ N: nNum, proportion: propNum, confidence: confNum, precision: precNum, designEffect: deNum });
        setResult({
          type: 'sampleSize', value: n,
          params: { N: nNum, proportion: propNum, confidence: confNum, precision: precNum, designEffect: deNum },
        });
      }
    } else {
      const nSamp = parseFloat(sampleSizeInput);
      if (!nSamp) return;
      const p = calcPrecisionProp({ N: nNum, proportion: propNum, confidence: confNum, sampleSize: nSamp, designEffect: deNum });
      setResult({
        type: 'precision', value: p.toFixed(3),
        params: { N: nNum, proportion: propNum, confidence: confNum, sampleSize: nSamp, designEffect: deNum },
      });
    }
    setShowResult(true);
  };
  ...
  const handleReset = () => {
    setN(''); setProportion(''); setConfidence('95'); setPrecision(''); setPrecMin(''); setPrecMax('');
  ...
  const canCalc =
    mode === 'sampleSize'
      ? Boolean(proportion && confidence && (useRange ? (precMin && precMax && precInc) : precision))
      : Boolean(proportion && sampleSizeInput && confidence);
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '28px 24px 60px' }}>
        ...
        <div className="anim-fadeRight" style={{
          ...
          <span style={{ color: '#d1d5db' }}>/</span>
          <span style={{ color: '#374151' }}>Proporción</span>
        </div>
        ...
        <div className="anim-fadeRight" style={{ display: 'flex', alignItems: 'flex-start', gap: 16, marginBottom: 8, animationDelay: '100ms' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 14, color: '#6b7280', margin: '4px 0 0', lineHeight: 1.5 }}>
              Intervalo de confianza — Calcula cuántos sujetos necesitas para estimar una proporción poblacional (prevalencia, tasa, porcentaje).
            </p>
          </div>
        </div>
        ...
        <div className="anim-cinematic" style={{
          ...
          <span>
            <b>Asistente IA:</b> ¿No conoces la proporción esperada? Puedo buscar prevalencias reportadas en la literatura para tu variable de interés.
          </span>
        </div>
        ...
        <div className="anim-cinematic" style={{
          ...
          <div style={{ marginBottom: 8 }}>
            ...
            <Field
              ...
              tooltip="Número total de individuos en la población. Si la población es muy grande o desconocida, déjalo vacío para asumir población infinita. Ejemplo: total de pacientes del hospital en un año."
              value={N} onChange={setN}
              placeholder="Ej: 12000 (dejar vacío si es infinita)"
              optional
            />
          </div>
          ...
          <div style={{ marginBottom: 8 }}>
            ...
            <Field
              label="Proporción esperada (p)"
              tooltip="El porcentaje esperado del evento que estás estudiando. Por ejemplo, si investigas la prevalencia de hipertensión y estudios previos reportan ~30%, ingresa 30. Si no tienes idea, usa 50% (máxima variabilidad, tamaño de muestra más conservador)."
              value={proportion} onChange={setProportion}
              placeholder="Ej: 30"
              suffix="%"
              hint={!proportion ? 'Usa 50% si no tienes una estimación previa — esto dará el tamaño de muestra más conservador.' : null}
            />
            {/* Proportion visual bar */}
            {proportion && <ProportionVisual value={proportion} />}
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 20, flexWrap: 'wrap' }}>
              ...
              {['10', '20', '30', '50'].map((v) => (
                <button key={v} onClick={() => setProportion(v)}
                  style={{
                    padding: '4px 14px', borderRadius: 8, border: 'none',
                    fontSize: 12, fontWeight: 600, cursor: 'pointer',
                    fontFamily: "'DM Sans', 'Segoe UI', sans-serif",
                    background: proportion === v ? '#ecfdf5' : '#f9fafb',
                    color:      proportion === v ? '#059669' : '#9ca3af',
            ...
            <Field
              ...
              tooltip="El porcentaje de certeza deseado. 95% es el estándar en la mayoría de estudios en ciencias de la salud."
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            {mode === 'sampleSize' ? (
              <>
                ...
                {!useRange ? (
                  <Field
                    ...
                    tooltip="El margen de error máximo aceptable en puntos porcentuales. Por ejemplo, si estimas una prevalencia del 30% y quieres que tu estimación esté entre 25% y 35%, la precisión es 5%."
                    value={precision} onChange={setPrecision}
                    placeholder="Ej: 5"
                    suffix="%"
                  />
                ) : (
                  <div style={{
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                      <Field label="Mínimo"     value={precMin} onChange={setPrecMin} placeholder="1"  suffix="%" tooltip="El valor más bajo de precisión a evaluar." />
                      <Field label="Máximo"     value={precMax} onChange={setPrecMax} placeholder="10" suffix="%" tooltip="El valor más alto de precisión a evaluar." />
                      <Field label="Incremento" value={precInc} onChange={setPrecInc} placeholder="1"  suffix="%" tooltip="El paso entre cada valor de precisión." />
                    </div>
                  </div>
                )}
              </>
              ...
              <Field
                ...
                tooltip="El número de sujetos que ya tienes o planeas reclutar. Se calculará la precisión que puedes alcanzar con ese tamaño."
          ...
          {showAdvanced && (
            <div style={{
              ...
              <Field
                ...
                tooltip="Factor de corrección para muestreos complejos (estratificado, conglomerados). Valor 1 = muestreo aleatorio simple. Valores mayores incrementan el tamaño de muestra requerido. Rango típico: 1.0 a 3.0."
                ...
                <div style={{ padding: '24px 28px 20px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 16, fontWeight: 600, color: '#059669' }}>
                      {result.type === 'sampleSize' ? 'sujetos necesarios' : 'precisión absoluta'}
                    </span>
                  </div>
                </div>
                <div style={{
                  ...
                  <span style={{ fontSize: 12, color: '#065f46' }}><b>p:</b> {result.params.proportion}%</span>
                  <span style={{ fontSize: 12, color: '#065f46' }}><b>q:</b> {(100 - result.params.proportion).toFixed(1)}%</span>
                ...
                <div style={{
                  ...
                  <div style={{ fontSize: 13, color: '#065f46', lineHeight: 1.6 }}>
                    {result.type === 'sampleSize' ? (
                      <>
                        <b>Interpretación IA:</b> Necesitas al menos <b>{result.value} sujetos</b> para estimar una proporción esperada del {result.params.proportion}%
                        con un nivel de confianza del {result.params.confidence}% y un margen de error de ±{result.params.precision} puntos porcentuales.
                        {result.params.N && <> Se aplicó corrección por población finita (N={result.params.N.toLocaleString()}).</>}
                        {result.params.designEffect > 1 && <> El efecto de diseño de {result.params.designEffect} incrementó el tamaño requerido.</>}
                        {' '}Es recomendable añadir un 10-20% adicional para compensar posibles pérdidas o datos incompletos.
                        {result.params.proportion === 50 && <> Al usar p=50%, obtuviste el tamaño de muestra más conservador posible — si la proporción real es diferente, el tamaño necesario sería menor.</>}
                      </>
                    ) : (
                      <>
                        <b>Interpretación IA:</b> Con una muestra de <b>{result.params.sampleSize} sujetos</b> y una proporción esperada del {result.params.proportion}%,
                        puedes estimar la proporción real con una precisión de <b>±{result.value} puntos porcentuales</b> al {result.params.confidence}% de confianza.
                        {' '}Evalúa si esta precisión es aceptable para los objetivos de tu estudio.
                      </>
                    )}
                  </div>
                </div>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <p style={{ fontSize: 13, color: '#065f46', margin: 0 }}>
                    Proporción esperada: <b>{result.params.proportion}%</b> · Confianza: <b>{result.params.confidence}%</b>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                    ...
                    <tbody>
                      {result.values.map((row, i) => (
                        ...
                        >
                          <td style={{ padding: '12px 28px', borderBottom: '1px solid #f3f4f6', color: '#374151', fontWeight: 500 }}>
                ...
                <div style={{
                  ...
                  <div style={{ fontSize: 13, color: '#065f46', lineHeight: 1.6 }}>
                    <b>Interpretación IA:</b> La tabla muestra cómo varía el tamaño de muestra según la precisión deseada.
                    {result.values.length > 1 && <>
                      {' '}Con la precisión más estricta (±{result.values[0].precision.toFixed(1)}%) necesitarías <b>{result.values[0].n.toLocaleString()} sujetos</b>,
                      mientras que con la más flexible (±{result.values[result.values.length - 1].precision.toFixed(1)}%) solo <b>{result.values[result.values.length - 1].n.toLocaleString()}</b>.
                    </>}
                    {' '}Elige la precisión que sea clínicamente relevante para tu estudio y factible para tu capacidad de reclutamiento.
                  </div>
                </div>
              </div>
            )}
        ...
        <div style={{
          ...
          <p style={{ fontSize: 12, color: '#9ca3af', margin: '10px 0 0', lineHeight: 1.5 }}>
            Donde Z<sub>α/2</sub> es el valor crítico de la distribución normal, p es la proporción esperada del evento, q = 1 − p, y E es la precisión absoluta deseada en puntos porcentuales.
          </p>
        </div>
