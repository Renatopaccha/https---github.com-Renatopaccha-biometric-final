import React, { useState, useMemo, type ReactNode, type ChangeEvent } from "react";
...
interface EstadisticosIndividuales {
  m1: number; sd1: number; n1: number;
  m2: number; sd2: number; n2: number;
}

interface ResultadosEmparejadas {
  m_diff: number; sd_diff: number; n_pares: number;
  nc: number;

  ic_lower: number; ic_upper: number;

  t_stat: number; t_df: number;
  p_bil: number; p_izq: number; p_der: number;

  calcBil: boolean; calcUIzq: boolean; calcUDer: boolean; calcIC: boolean;
  var1Label: string; var2Label: string;
  
  // Opcional: para mostrar la tabla individual si vino de datos Excel
  indiv?: EstadisticosIndividuales;
}

interface InferenciaMediasEmparejadasProps {
...
/* ═══════════════════════════════════════════════════════════════════════════
   LÓGICA PRINCIPAL: COMPARACIÓN DE MEDIAS EMPAREJADAS
   ═══════════════════════════════════════════════════════════════════════════ */

function calcMediasEmparejadas(
  m_diff: number, sd_diff: number, n_pares: number, nc: number,
  calcBil: boolean, calcUIzq: boolean, calcUDer: boolean, calcIC: boolean,
  var1Label: string, var2Label: string,
  indiv?: EstadisticosIndividuales
): ResultadosEmparejadas {
  
  const alpha = (100 - nc) / 100;
  const t_df = n_pares - 1;
  const SE = sd_diff / Math.sqrt(n_pares);
  const t_stat = m_diff / SE;

  const tcrit = tinv(1 - alpha/2, t_df);
  const ic_lower = m_diff - tcrit * SE;
  const ic_upper = m_diff + tcrit * SE;

  const p_bil = 2 * (1 - tcdf(Math.abs(t_stat), t_df));
  const p_izq = tcdf(t_stat, t_df);
  const p_der = 1 - tcdf(t_stat, t_df);

  return {
    m_diff, sd_diff, n_pares, nc,
    ic_lower, ic_upper,
    t_stat, t_df, p_bil, p_izq, p_der,
...
function buildResumenIA(res: ResultadosEmparejadas): string {
  let txt = `Inferencia. Comparación de medias emparejadas\n`;
  if (res.var1Label !== "Muestra 1") {
    txt += `Variables: ${res.var1Label} vs ${res.var2Label}\n\n`;
  }
  txt += `Diferencia de medias (μd) = ${fmt3(res.m_diff)}\n`;
  txt += `Desviación estándar de diferencias = ${fmt3(res.sd_diff)}\n`;
  txt += `Número de pares (n) = ${fmtN(res.n_pares)}\n\n`;
  
  if (res.calcIC) {
    txt += `IC ${res.nc}% para la Diferencia: [${fmt3(res.ic_lower)} , ${fmt3(res.ic_upper)}]\n`;
  }
  if (res.calcBil) {
    txt += `Prueba T para muestras dependientes (Bilateral) p-valor = ${fmtP(res.p_bil)}\n`;
  }
  txt += `\nINSTRUCCIONES PARA LA IA: Analiza si existe un cambio o diferencia significativa entre las mediciones emparejadas basándote en el p-valor y si el Intervalo de Confianza cruza el cero. Explica clínicamente si el segundo momento (o variable 2) representa un aumento o descenso respecto a la variable 1. Responde en prosa académica y concisa.`;
  return txt;
}
...
async function exportarExcel(res: ResultadosEmparejadas): Promise<void> {
  ...
  html += `<tr><th colspan="4" style="background-color:#0F766E; color:white; font-size:14px; padding:8px; border:1px solid #000;">Inferencia. Comparación de medias emparejadas</th></tr>`;
  html += `<tr><td colspan="4"></td></tr>`;

  if (res.indiv) {
    html += `<tr><th style="${cssTh}"></th><th style="${cssTh}">${res.var1Label}</th><th style="${cssTh}">${res.var2Label}</th><th></th></tr>`;
    html += `<tr><td style="${cssTd}">Media</td><td style="${cssTdNum}">${fmt3(res.indiv.m1).replace(".", ",")}</td><td style="${cssTdNum}">${fmt3(res.indiv.m2).replace(".", ",")}</td><td></td></tr>`;
    html += `<tr><td style="${cssTd}">Desviación estándar</td><td style="${cssTdNum}">${fmt3(res.indiv.sd1).replace(".", ",")}</td><td style="${cssTdNum}">${fmt3(res.indiv.sd2).replace(".", ",")}</td><td></td></tr>`;
    html += `<tr><td style="${cssTd}">Tamaño de muestra</td><td style="${cssTdNum}">${fmtN(res.indiv.n1)}</td><td style="${cssTdNum}">${fmtN(res.indiv.n2)}</td><td></td></tr>`;
    html += `<tr><td colspan="4"></td></tr>`;
  }

  html += `<tr><td colspan="4" style="font-weight:bold; ${cssTd}">Diferencia de medias: ${fmt3(res.m_diff).replace(".", ",")}</td></tr>`;
  html += `<tr><td colspan="4" style="font-weight:bold; ${cssTd}">Desviación estándar: ${fmt3(res.sd_diff).replace(".", ",")}</td></tr>`;
  html += `<tr><td colspan="4" style="font-weight:bold; ${cssTd}">Número de pares: ${fmtN(res.n_pares)}</td></tr>`;
  html += `<tr><td colspan="4" style="font-weight:bold; ${cssTd}">Nivel de confianza: ${res.nc},0%</td></tr>`;
  html += `<tr><td colspan="4"></td></tr>`;
  
  html += `<tr><td colspan="4" style="font-weight:bold; text-decoration: underline;">Resultados:</td></tr>`;

  if (res.calcIC) {
    html += `<tr><td colspan="4">Intervalo de confianza (${res.nc},0%)</td></tr>`;
    html += `<tr><th style="${cssTh}">Diferencia de medias</th><th style="${cssTh}">Límite inferior</th><th style="${cssTh}">Límite superior</th><th></th></tr>`;
    html += `<tr><td style="${cssTdNum}">${fmt3(res.m_diff).replace(".", ",")}</td><td style="${cssTdNum}">${fmt3(res.ic_lower).replace(".", ",")}</td><td style="${cssTdNum}">${fmt3(res.ic_upper).replace(".", ",")}</td><td></td></tr>`;
    html += `<tr><td colspan="4"></td></tr>`;
  }

  if (res.calcBil || res.calcUIzq || res.calcUDer) {
    html += `<tr><td colspan="4">Prueba de comparación de medias</td></tr>`;
    html += `<tr><th style="${cssTh}">Contraste</th><th style="${cssTh}">Estadístico t</th><th style="${cssTh}">gl</th><th style="${cssTh}">Valor p</th></tr>`;
    if (res.calcBil) html += `<tr><td style="${cssTd}">Bilateral</td><td style="${cssTdNum}">${fmt3(res.t_stat).replace(".", ",")}</td><td style="${cssTdNum}">${fmtGl(res.t_df)}</td><td style="${cssTdNum}">${fmtP(res.p_bil)}</td></tr>`;
    if (res.calcUIzq) html += `<tr><td style="${cssTd}">Unilateral izquierdo</td><td style="${cssTdNum}">${fmt3(res.t_stat).replace(".", ",")}</td><td style="${cssTdNum}">${fmtGl(res.t_df)}</td><td style="${cssTdNum}">${fmtP(res.p_izq)}</td></tr>`;
    if (res.calcUDer) html += `<tr><td style="${cssTd}">Unilateral derecho</td><td style="${cssTdNum}">${fmt3(res.t_stat).replace(".", ",")}</td><td style="${cssTdNum}">${fmtGl(res.t_df)}</td><td style="${cssTdNum}">${fmtP(res.p_der)}</td></tr>`;
  ...
  a.href = url; a.download = `comparacion_medias_emparejadas_${Date.now()}.xls`;
  a.click(); URL.revokeObjectURL(url);
}

function exportarWord(res: ResultadosEmparejadas): void {
  const css = {
  ...
  let html = `<h2 style="${css.h2}">Inferencia. Comparación de medias emparejadas</h2>`;
  
  if (res.indiv) {
    html += `<table style="${css.tbl}"><thead><tr><th style="${css.th}"></th><th style="${css.th}">${res.var1Label}</th><th style="${css.th}">${res.var2Label}</th></tr></thead><tbody>`;
    html += `<tr><td style="${css.td0}">Media</td><td style="${css.td}">${fmt3(res.indiv.m1).replace(".", ",")}</td><td style="${css.td}">${fmt3(res.indiv.m2).replace(".", ",")}</td></tr>`;
    html += `<tr><td style="${css.td0}">Desviación estándar</td><td style="${css.td}">${fmt3(res.indiv.sd1).replace(".", ",")}</td><td style="${css.td}">${fmt3(res.indiv.sd2).replace(".", ",")}</td></tr>`;
    html += `<tr><td style="${css.td0}">Tamaño de muestra</td><td style="${css.td}">${fmtN(res.indiv.n1)}</td><td style="${css.td}">${fmtN(res.indiv.n2)}</td></tr>`;
    html += `</tbody></table>`;
  }
  
  html += `<p style="${css.p}">Diferencia de medias: ${fmt3(res.m_diff).replace(".", ",")}</p>`;
  html += `<p style="${css.p}">Desviación estándar: ${fmt3(res.sd_diff).replace(".", ",")}</p>`;
  html += `<p style="${css.p}">Número de pares: ${fmtN(res.n_pares)}</p>`;
  ...
  if (res.calcIC) {
    ...
    html += `<table style="${css.tbl}"><thead><tr><th style="${css.th}">Diferencia de medias</th><th style="${css.th}">Límite inferior</th><th style="${css.th}">Límite superior</th></tr></thead><tbody>`;
    html += `<tr><td style="${css.td}">${fmt3(res.m_diff).replace(".", ",")}</td><td style="${css.td}">${fmt3(res.ic_lower).replace(".", ",")}</td><td style="${css.td}">${fmt3(res.ic_upper).replace(".", ",")}</td></tr>`;
    html += `</tbody></table>`;
  }

  if (res.calcBil || res.calcUIzq || res.calcUDer) {
    html += `<p style="${css.p}">Prueba de comparación de medias</p>`;
    ...
    if (res.calcBil) html += `<tr><td style="${css.td0}">Bilateral</td><td style="${css.td}">${fmt3(res.t_stat).replace(".", ",")}</td><td style="${css.td}">${fmtGl(res.t_df)}</td><td style="${css.td}">${fmtP(res.p_bil)}</td></tr>`;
    if (res.calcUIzq) html += `<tr><td style="${css.td0}">Unilateral izquierdo</td><td style="${css.td}">${fmt3(res.t_stat).replace(".", ",")}</td><td style="${css.td}">${fmtGl(res.t_df)}</td><td style="${css.td}">${fmtP(res.p_izq)}</td></tr>`;
    if (res.calcUDer) html += `<tr><td style="${css.td0}">Unilateral derecho</td><td style="${css.td}">${fmt3(res.t_stat).replace(".", ",")}</td><td style="${css.td}">${fmtGl(res.t_df)}</td><td style="${css.td}">${fmtP(res.p_der)}</td></tr>`;
  ...
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url; a.download = `comparacion_medias_emparejadas_${Date.now()}.doc`;
  a.click(); URL.revokeObjectURL(url);
}
...
export default function InferenciaMediasEmparejadas({
  datosExcel = null,
  loadingExcel = false,
  onBack,
  onContinuarChat = null,
}: InferenciaMediasEmparejadasProps) {
  ...
  const [manMDiff, setManMDiff] = useState("");
  const [manSdDiff, setManSdDiff] = useState("");
  const [manNPares, setManNPares] = useState("");
  ...
  const [res, setRes] = useState<ResultadosEmparejadas | null>(null);
  ...
  const [miniAnalysisOpen, setMiniAnalysisOpen] = useState(false);
  ...
  const autoStats = useMemo(() => {
    ...
    // Solo tomamos PARES VALIDOS (donde ambos tienen dato)
    filtered.forEach(r => {
      const v1 = parseNumericContinuous(r[colVar1]);
      const v2 = parseNumericContinuous(r[colVar2]);
      if (v1 !== "missing" && v1 !== "invalid" && v2 !== "missing" && v2 !== "invalid") {
        arr1.push(v1 as number);
        arr2.push(v2 as number);
        diffs.push((v1 as number) - (v2 as number));
      }
    });
    ...
    const st1 = calc(arr1);
    const st2 = calc(arr2);
    const stDiff = calc(diffs);

    if (stDiff.n < 2) return null;
  ...
  }, [datosExcel, colVar1, colVar2, filterRules, filterCombo]);

  const efectivos = useMemo(() => {
    if (modo === "individual") {
        return autoStats;
    } else {
        const m_diff = parseFloat(manMDiff.replace(",", "."));
        const sd_diff = parseFloat(manSdDiff.replace(",", "."));
        const n_pares = parseInt(manNPares.replace(/\./g, ""), 10);

        if (isFinite(m_diff) && isFinite(sd_diff) && isFinite(n_pares) && n_pares > 1 && sd_diff >= 0) {
            return { m_diff, sd_diff, n_pares, v1Label: "Muestra 1", v2Label: "Muestra 2", indiv: undefined };
        }
        return null;
    }
  }, [modo, autoStats, manMDiff, manSdDiff, manNPares]);
  ...
  function handleCalc() {
    ...
    setLoad(true); setErr(""); setRes(null); setIaOpen(false); setIaText(""); setMiniAnalysisOpen(false);

    setTimeout(() => {
      try {
        const { m_diff, sd_diff, n_pares, v1Label, v2Label, indiv } = efectivos!;
        const resultados = calcMediasEmparejadas(
            m_diff, sd_diff, n_pares, nivelConf,
            calcBil, calcUIzq, calcUDer, calcIC,
            v1Label, v2Label, indiv
        );
        setRes(resultados);
      } catch (ex: unknown) {
  ...
  function handleReset() {
    setColVar1(""); setColVar2(""); setFilterRules([]);
    setManMDiff(""); setManSdDiff(""); setManNPares("");
    setNivel(95);
    setCalcIC(true); setCalcBil(true); setCalcUIzq(false); setCalcUDer(false);
    setRes(null); setErr(""); setIaOpen(false); setIaText(""); setMiniAnalysisOpen(false);
  }
    ...
    try {
      ...
      const resp = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: "user",
            content: "Eres un bioestadístico experto. Interpreta los resultados de medias emparejadas para una tesis clínica en máximo 300 palabras.\n\n" + resumen,
          }],
        }),
      });
    ...
    <div style={{ fontFamily: "'DM Sans','Segoe UI',sans-serif", background: "#f4f6f8", minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 920, margin: "0 auto", padding: "28px 24px 60px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 22, fontSize: 13, color: "#6b7280", fontWeight: 500 }}>
          ...
          <span style={{ color: "#111827", fontWeight: 600 }}>Comparación de medias emparejadas</span>
        </div>
        ...
        <div style={{ background: "white", borderRadius: "16px 16px 0 0", padding: "28px 32px 0" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <h1 style={{ fontSize: 24, fontWeight: 800, color: "#111827", margin: 0, letterSpacing: "-.02em" }}>
              Inferencia. Comparación de medias emparejadas
            </h1>
          </div>
          <p style={{ color: "#6b7280", fontSize: 14, margin: "6px 0 0 14px", lineHeight: 1.5, paddingBottom: 20 }}>
            Análisis de muestras dependientes o relacionadas. Útil para medir diferencias en un mismo grupo antes y después de una intervención.
          </p>
        </div>
        ...
        <div style={{ background: "linear-gradient(135deg,#ecfdf5,#f0fdf4)", borderTop: "1px solid #a7f3d0", borderBottom: "1px solid #a7f3d0", padding: "13px 22px", display: "flex", alignItems: "flex-start", gap: 10, fontSize: 13, color: "#065f46" }}>
          ...
          <span style={{ lineHeight: 1.65 }}>
            <b>¿Para qué sirve?</b> Analiza si existe un cambio significativo entre dos mediciones realizadas en el mismo sujeto (ej. Peso basal vs Peso post-tratamiento). Se calcula la "diferencia" intragrupo para evaluar si estadísticamente se aleja del valor 0 mediante la prueba t de Student pareada.
          </span>
        </div>
        ...
        <div style={{ background: "white", borderRadius: "0 0 16px 16px", borderTop: "1px solid #e5e7eb", padding: "26px 28px 24px", boxShadow: "0 2px 10px rgba(0,0,0,.05)", marginBottom: 14 }}>
          ...
          {modo === "individual" ? (
             <div style={{ marginBottom: 20 }}>
               <StepLabel step="Paso 1" label="Selección de variables pareadas" />
               {!datosExcel ? (
                  ...
                  <div style={{ padding: "13px 16px", background: "#fef2f2", border: "1.5px solid #fca5a5", borderRadius: 10, fontSize: 13, color: "#991b1b", display: "flex", alignItems: "center", gap: 9 }}>
                    <Icon d={IC_SVG.warn} size={14} />
                    Se necesitan al menos 2 variables numéricas continuas para emparejar.
                  </div>
                ) : (
                  <div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                      <div style={{ position: "relative" }}>
                        <label style={{ fontSize: 13, fontWeight: 600, color: "#374151", display: "block", marginBottom: 7 }}>Variable 1 (Ej. Momento Basal):</label>
                        <select value={colVar1} onChange={(e) => setColVar1(e.target.value)} style={{ width: "100%", padding: "11px 36px 11px 14px", border: `2px solid ${colVar1 ? "#0d9488" : "#e5e7eb"}`, borderRadius: 10, fontSize: 14, fontFamily: "inherit", appearance: "none", color: colVar1 ? "#111827" : "#9ca3af", cursor: "pointer", outline: "none", background: colVar1 ? "#f0fdf4" : "white", transition: "all .2s" }}>
                      ...
                      <div style={{ position: "relative" }}>
                        <label style={{ fontSize: 13, fontWeight: 600, color: "#374151", display: "block", marginBottom: 7 }}>Variable 2 (Ej. Momento Final):</label>
                        <select value={colVar2} onChange={(e) => setColVar2(e.target.value)} style={{ width: "100%", padding: "11px 36px 11px 14px", border: `2px solid ${colVar2 ? "#0d9488" : "#e5e7eb"}`, borderRadius: 10, fontSize: 14, fontFamily: "inherit", appearance: "none", color: colVar2 ? "#111827" : "#9ca3af", cursor: "pointer", outline: "none", background: colVar2 ? "#f0fdf4" : "white", transition: "all .2s" }}>
                          <option value="">Seleccionar variable...</option>
                          {colsCuant.map((c: string) => <option key={c} value={c}>{c}</option>)}
                        </select>
                      </div>
                ...
                {autoStats && (
                  <div style={{ marginTop: 20, background: "#f0fdf4", border: "1.5px solid #a7f3d0", borderRadius: 12, padding: "16px 20px" }}>
                    <div style={{ fontSize: 12, color: "#065f46", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".05em", marginBottom: 12, display: "flex", alignItems: "center", gap: 6 }}>
                      <Icon d={IC_SVG.check} size={14} /> Descriptivos de pares válidos
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                      <div>
                         <div style={{ display: "grid", gridTemplateColumns: "100px 1fr 1fr", gap: 10, alignItems: "center" }}>
                           <div></div>
                           <div style={{ fontWeight: 700, color: "#0f766e", fontSize: 13, borderBottom: "1px solid #a7f3d0", paddingBottom: 4 }}>{autoStats.v1Label}</div>
                           <div style={{ fontWeight: 700, color: "#0f766e", fontSize: 13, borderBottom: "1px solid #a7f3d0", paddingBottom: 4 }}>{autoStats.v2Label}</div>
                           
                           <div style={{ fontWeight: 600, color: "#374151", fontSize: 12 }}>Media</div>
                           <div style={{ fontFamily: "'DM Mono', monospace", color: "#111827", fontSize: 14 }}>{fmt3(autoStats.indiv.m1)}</div>
                           <div style={{ fontFamily: "'DM Mono', monospace", color: "#111827", fontSize: 14 }}>{fmt3(autoStats.indiv.m2)}</div>
                           
                           <div style={{ fontWeight: 600, color: "#374151", fontSize: 12 }}>Desv. est.</div>
                           <div style={{ fontFamily: "'DM Mono', monospace", color: "#111827", fontSize: 14 }}>{fmt3(autoStats.indiv.sd1)}</div>
                           <div style={{ fontFamily: "'DM Mono', monospace", color: "#111827", fontSize: 14 }}>{fmt3(autoStats.indiv.sd2)}</div>

                           <div style={{ fontWeight: 600, color: "#374151", fontSize: 12 }}>Tamaño (n)</div>
                           <div style={{ fontFamily: "'DM Mono', monospace", color: "#111827", fontSize: 14 }}>{fmtN(autoStats.indiv.n1)}</div>
                           <div style={{ fontFamily: "'DM Mono', monospace", color: "#111827", fontSize: 14 }}>{fmtN(autoStats.indiv.n2)}</div>
                         </div>
                      </div>
                      <div style={{ borderLeft: "1px dashed #a7f3d0", paddingLeft: 20, display: "flex", flexDirection: "column", justifyContent: "center" }}>
                         <div style={{ fontWeight: 700, color: "#0f766e", fontSize: 13, marginBottom: 8 }}>Diferencias apareadas (d = X1 - X2)</div>
                         <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 6, fontSize: 13 }}>
                           <span style={{ color: "#374151" }}>Diferencia de medias:</span> <span style={{ fontFamily: "'DM Mono', monospace", fontWeight: 700 }}>{fmt3(autoStats.m_diff)}</span>
                           <span style={{ color: "#374151" }}>Desviación estándar:</span> <span style={{ fontFamily: "'DM Mono', monospace", fontWeight: 700 }}>{fmt3(autoStats.sd_diff)}</span>
                           <span style={{ color: "#374151" }}>Número de pares:</span> <span style={{ fontFamily: "'DM Mono', monospace", fontWeight: 700 }}>{fmtN(autoStats.n_pares)}</span>
                         </div>
                      </div>
                    </div>
                  </div>
                )}
                {colVar1 && colVar2 && !autoStats && (
                  <p style={{ fontSize: 12, color: "#ef4444", marginTop: 8 }}>⚠ Las columnas seleccionadas no poseen suficientes pares válidos compartidos (n {">"} 1).</p>
                )}
             </div>
          ) : (
            <div style={{ marginBottom: 20 }}>
              <StepLabel step="Paso 1" label="Datos resumidos (Diferencias)" />
              <p style={{ fontSize: 13, color: "#6b7280", margin: "0 0 14px", lineHeight: 1.5 }}>
                Ingrese los estadísticos descriptivos resultantes de la diferencia de los pares (<i>d<sub>i</sub></i> = <i>x<sub>1i</sub></i> − <i>x<sub>2i</sub></i>).
              </p>
              <div style={{ border: "1.5px solid #e5e7eb", borderRadius: 12, padding: "16px 20px", background: "#f9fafb" }}>
                <div style={{ display: "grid", gridTemplateColumns: "160px 150px", gap: 14, alignItems: "center" }}>
                  <div style={{ fontWeight: 600, color: "#374151", fontSize: 13 }}>Diferencia de medias</div>
                  <input type="text" value={manMDiff} onChange={e => setManMDiff(e.target.value)} placeholder="46,938" style={{ padding: "8px", border: "1.5px solid #d1d5db", borderRadius: 8, fontFamily: "'DM Mono', monospace", textAlign: "right", outline: "none" }} />

                  <div style={{ fontWeight: 600, color: "#374151", fontSize: 13 }}>Desviación estándar</div>
                  <input type="text" value={manSdDiff} onChange={e => setManSdDiff(e.target.value)} placeholder="18,811" style={{ padding: "8px", border: "1.5px solid #d1d5db", borderRadius: 8, fontFamily: "'DM Mono', monospace", textAlign: "right", outline: "none" }} />

                  <div style={{ fontWeight: 600, color: "#374151", fontSize: 13 }}>Número de pares</div>
                  <input type="text" value={manNPares} onChange={e => setManNPares(e.target.value)} placeholder="2.000" style={{ padding: "8px", border: "1.5px solid #d1d5db", borderRadius: 8, fontFamily: "'DM Mono', monospace", textAlign: "right", outline: "none" }} />
                </div>
              </div>
            </div>
          )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <div style={{ border: "1.5px solid #e5e7eb", borderRadius: 12, padding: "14px 16px", background: "#fafbfc" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <CheckRow checked={calcBil} onChange={setCalcBil} label={<>Bilateral &nbsp;<span style={{ color: "#9ca3af", fontWeight: 400, fontSize: 12 }}>( H₀: μ<sub>d</sub> = 0 vs. H₁: μ<sub>d</sub> ≠ 0 )</span></>} indented />
                <CheckRow checked={calcUIzq} onChange={setCalcUIzq} label={<>Unilateral izquierdo &nbsp;<span style={{ color: "#9ca3af", fontWeight: 400, fontSize: 12 }}>( H₀: μ<sub>d</sub> = 0 vs. H₁: μ<sub>d</sub> &lt; 0 )</span></>} indented />
                <CheckRow checked={calcUDer} onChange={setCalcUDer} label={<>Unilateral derecho &nbsp;<span style={{ color: "#9ca3af", fontWeight: 400, fontSize: 12 }}>( H₀: μ<sub>d</sub> = 0 vs. H₁: μ<sub>d</sub> &gt; 0 )</span></>} indented />
              </div>
            </div>
          </div>
          ...
          <div style={{ marginTop: 30, animation: "slideUp .4s cubic-bezier(.16,1,.3,1)" }}>
            ...
            <div style={{ background: "white", borderRadius: 14, border: "1.5px solid #e5e7eb", padding: "16px 22px", marginBottom: 16, display: "flex", flexWrap: "wrap", gap: 14, alignItems: "center", boxShadow: "0 1px 4px rgba(0,0,0,.04)" }}>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 16, fontSize: 13 }}>
                <span style={{ color: "#6b7280" }}>Variables: <b style={{ fontFamily: "'DM Mono', monospace", color: "#111827" }}>{res.var1Label} vs {res.var2Label}</b></span>
              </div>
            ...
            {iaOpen && (
              <div style={{ marginBottom: 16, background: "linear-gradient(135deg,#fdf4ff,#ede9fe)", border: "2px solid #c4b5fd", borderRadius: 16, padding: 22, animation: "slideUp .3s ease" }}>
                ...
                <div style={{ background: "white", borderRadius: 12, border: "1px solid #ddd6fe", padding: "18px 22px", minHeight: 140, maxHeight: 380, overflowY: "auto", lineHeight: 1.8, fontSize: 14, color: "#374151" }}>
                  {iaLoad ? <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 110, gap: 12 }}><Spin /><span style={{ color: "#9ca3af", fontSize: 13 }}>Evaluando diferencias de promedios...</span></div> : <div style={{ whiteSpace: "pre-wrap" }}>{iaText}</div>}
                </div>
                {iaText && !iaLoad && (
                  <div style={{ marginTop: 12, display: "flex", justifyContent: "flex-end" }}>
                    <button onClick={() => onContinuarChat?.(`Hola, sobre el análisis de T de Student de medias emparejadas, ¿podemos ver en profundidad si el intervalo de confianza [${fmt3(res.ic_lower)}, ${fmt3(res.ic_upper)}] refleja significancia clínica?`)} className="hov-btn" style={{ padding: "9px 18px", borderRadius: 10, border: "2px solid #7c3aed", cursor: "pointer", fontSize: 13, fontWeight: 700, fontFamily: "inherit", background: "#7c3aed", color: "white", display: "flex", alignItems: "center", gap: 7 }}>
            ...
            {/* TABLA DE DESCRIPTIVOS INDIVIDUALES SI APLICA */}
            {res.indiv && (
              <TablaAcademica
                headers={["", res.var1Label, res.var2Label]}
                filas={[
                  [{ v: "Media", align: "left" }, { v: fmt3(res.indiv.m1).replace(".", ","), mono: true, align: "right" }, { v: fmt3(res.indiv.m2).replace(".", ","), mono: true, align: "right" }],
                  [{ v: "Desviación estándar", align: "left" }, { v: fmt3(res.indiv.sd1).replace(".", ","), mono: true, align: "right" }, { v: fmt3(res.indiv.sd2).replace(".", ","), mono: true, align: "right" }],
                  [{ v: "Tamaño de muestra", align: "left" }, { v: fmtN(res.indiv.n1), mono: true, align: "right" }, { v: fmtN(res.indiv.n2), mono: true, align: "right" }]
                ]}
              />
            )}

            <div style={{ background: "#f8fafc", border: "1.5px solid #e5e7eb", borderRadius: 12, padding: "16px 20px", marginBottom: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                <div style={{ fontSize: 12, color: "#4b5563", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".05em", display: "flex", alignItems: "center", gap: 6 }}>
                  <Icon d={IC_SVG.info} size={14} /> Resumen de diferencias
                </div>
                <button onClick={() => setMiniAnalysisOpen(!miniAnalysisOpen)} className="hov-btn" style={{ background: "#e0f2fe", border: "1px solid #bae6fd", borderRadius: 6, padding: "5px 12px", cursor: "pointer", color: "#0369a1", display: "flex", alignItems: "center", gap: 6, fontSize: 11, fontWeight: 700 }}>
                  <Icon d={IC_SVG.info} size={12} strokeWidth="3" /> {miniAnalysisOpen ? "Ocultar análisis" : "Análisis rápido"}
                </button>
              </div>

              {miniAnalysisOpen && (
                <div style={{ background: "white", border: "1px solid #e5e7eb", borderLeft: "4px solid #0ea5e9", borderRadius: 6, padding: "14px 18px", marginBottom: 18, fontSize: 13, color: "#374151", lineHeight: 1.6, animation: "fadeIn .3s ease" }}>
                  {res.calcIC && (
                    <p style={{ margin: "0 0 8px 0" }}>
                      ➤ El <b>intervalo de confianza</b> al {res.nc}% va de {fmt3(res.ic_lower).replace(".", ",")} a {fmt3(res.ic_upper).replace(".", ",")}. Como este intervalo <b>{res.ic_lower <= 0 && res.ic_upper >= 0 ? "INCLUYE el cero" : "NO incluye el cero"}</b>, se deduce que la diferencia promedio observada <b>{res.ic_lower <= 0 && res.ic_upper >= 0 ? "NO es" : "SÍ es"} estadísticamente significativa</b>.
                    </p>
                  )}
                  {res.calcBil && (
                    <p style={{ margin: 0 }}>
                      ➤ El <b>valor p bilateral</b> es {fmtP(res.p_bil)}, el cual es <b>{res.p_bil < (100 - res.nc) / 100 ? "MENOR" : "MAYOR"}</b> al nivel de significancia de {(100 - res.nc) / 100}. Esto indica que el cambio entre las mediciones emparejadas <b>{res.p_bil < (100 - res.nc) / 100 ? "representa un efecto real en la población" : "podría deberse simplemente al azar"}</b>.
                    </p>
                  )}
                  {!res.calcIC && !res.calcBil && (
                    <p style={{ margin: 0, color: "#6b7280", fontStyle: "italic" }}>Seleccione "Intervalo de confianza" o "Contraste Bilateral" para generar un análisis estadístico.</p>
                  )}
                </div>
              )}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 16 }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  <span style={{ fontSize: 12, color: "#6b7280", fontWeight: 600 }}>Diferencia de medias</span>
                  <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 16, fontWeight: 800, color: "#0f766e" }}>{fmt3(res.m_diff).replace(".", ",")}</span>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  <span style={{ fontSize: 12, color: "#6b7280", fontWeight: 600 }}>Desviación estándar</span>
                  <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 16, fontWeight: 800, color: "#111827" }}>{fmt3(res.sd_diff).replace(".", ",")}</span>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  <span style={{ fontSize: 12, color: "#6b7280", fontWeight: 600 }}>Número de pares</span>
                  <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 16, fontWeight: 800, color: "#111827" }}>{fmtN(res.n_pares)}</span>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  <span style={{ fontSize: 12, color: "#6b7280", fontWeight: 600 }}>Nivel de confianza</span>
                  <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 16, fontWeight: 800, color: "#111827" }}>{res.nc},0%</span>
                </div>
              </div>
            </div>
            ...
            {res.calcIC && (
              <TablaAcademica
                ...
                headers={["Diferencia de medias", "Límite inferior", "Límite superior"]}
                filas={[[
                  { v: fmt3(res.m_diff).replace(".", ","), mono: true, align: "right" },
                  { v: fmt3(res.ic_lower).replace(".", ","), mono: true, align: "right" },
                  { v: fmt3(res.ic_upper).replace(".", ","), mono: true, align: "right" }
                ]]}
              />
            )}
            ...
            {algunContrastes && (
              <TablaAcademica
                titulo="Prueba de comparación de medias"
                headers={["Contraste", "Estadístico t", "gl", "Valor p"]}
                filas={[
                  ...(res.calcBil ? [[{ v: "Bilateral", align: "left" as const }, { v: fmt3(res.t_stat).replace(".", ","), mono: true, align: "right" as const }, { v: fmtGl(res.t_df), mono: true, align: "right" as const }, { v: fmtP(res.p_bil), mono: true, bold: true, align: "right" as const }]] : []),
                  ...(res.calcUIzq ? [[{ v: "Unilateral izquierdo", align: "left" as const }, { v: fmt3(res.t_stat).replace(".", ","), mono: true, align: "right" as const }, { v: fmtGl(res.t_df), mono: true, align: "right" as const }, { v: fmtP(res.p_izq), mono: true, bold: true, align: "right" as const }]] : []),
                  ...(res.calcUDer ? [[{ v: "Unilateral derecho", align: "left" as const }, { v: fmt3(res.t_stat).replace(".", ","), mono: true, align: "right" as const }, { v: fmtGl(res.t_df), mono: true, align: "right" as const }, { v: fmtP(res.p_der), mono: true, bold: true, align: "right" as const }]] : [])
                ]}
